...
${compactRoutes}`;
    ...
    const modeRule = "这是副图迁移任务：在内部推理时，A 副图只负责提供已经分析出的场景、构图、信息表达和商业视觉机制；最终生图阶段附带的参考图片是用户自己的产品身份图，其中的真实商品才是唯一商品主体。A 图中的竞品产品绝不能进入新图。A、B 只是本次工作流的内部角色代号，最终提示词正文不得出现 A 图、A 副图、B 图、B 产品或类似代号。";
...
- 明确写出“唯一附加图片是用户自己的产品身份参考图，只锁定其中真实商品本身，不锁定该图片原有的背景和构图”。以该真实商品为画面唯一商品主体，保留其真实外观、品牌、包装结构、颜色、标签位置、比例和确实可读的文字。
- 根据本方案重建完整副图画面，允许改变真实商品在画面中的位置、大小、背景、机位关系、场景、光影、道具和外部信息组织，但不得重绘或改版商品包装。
- 只执行本方案已经写明的视觉机制，不提及方案来自哪张图；不复制视觉参考中竞品的品牌、产品名、包装、文案、规格、功效或图形文字，不虚构产品身份参考图中不存在的信息。

每条提示词要求：
- 先用主要篇幅写本方案独有的可见设计机制、画面关系、信息组织、材质/图形协作和观看结果，再写共同底线。
- 外部标题或卖点只能写成从唯一附加产品参考图中可确认信息提取的内容槽位，不得提前编造具体产品文案；若无法确认则该位置留空。
- 提示词本身继续使用中文描述设计，但必须明确规定：画面中新增加的标题、卖点、图标说明和短标签全部使用英文，禁止出现中文、拼音或中英混排。优先从最终附加产品参考图中确实可读的英文信息提取；当前阶段看不到该产品图，不得把中文短语当作最终画面文案。方案可以说明信息槽位的语义目的，但只有当产品标签支持相同含义时才允许在生图阶段转写成英文，否则该位置使用无文字图标或留空。
- 描述一个确定成片，不写“如果、或者、可选择”等备选表达，不输出分析报告、模型、API 或文件名。
- 最终五条正文不得出现“A”“B”角色代号，也不得解释参考图分工；视觉参考的作用必须完全转换成具体的场景、构图、信息组织、材质和观看路径描述。
