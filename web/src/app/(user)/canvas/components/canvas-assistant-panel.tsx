                    ...
                    {view === "history" ? (
                        ...
                        <div className="flex h-full flex-col items-center justify-center px-1 text-center">
                            <div className="relative font-serif text-4xl font-bold italic tracking-normal" style={{ color: theme.node.text }}>
                                <span>EONS AI Image Studio</span>
                                <DiaTextReveal className="absolute inset-0" colors={["#A97CF8", "#F38CB8", "#FDCC92"]} textColor="transparent" duration={1.8} startOnView={false} text="EONS AI Image Studio" />
                            </div>
                            <div className="mt-3 font-serif text-base italic tracking-wide opacity-60">Create, connect, refine</div>
                        </div>
                    )}
                </div>
            )}
...
function formatOnlineLogText(logs: OnlineAgentLog[], context: OnlineAgentLogContext) {
    const head = [
        "EONS AI Image Studio Agent 诊断日志",
...
function textNodeOp(input: Record<string, unknown>, x: number, y: number): CanvasAgentOp {
    return { type: "add_node", id: stringOptional(input.id), nodeType: CanvasNodeType.Text, title: stringOptional(input.title), position: { x, y }, width: numberOptional(input.width), height: numberOptional(input.height), metadata: { content: stringOptional(input.text), status: "success", fontSize: 16 } };
}
...
function generationCount(value: string) {
    return Math.max(1, Math.min(3, Math.floor(Math.abs(Number(value)) || 1)));
}
